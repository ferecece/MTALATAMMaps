...
addEventHandler("onPlayerFinish", getRootElement(), function(rank, time)
	...
	if (DATABASE) then
		if getPedOccupiedVehicle(source) then
			...
			sortQuery = dbQuery(DATABASE, "SELECT * FROM BadlandsTable ORDER BY score ASC LIMIT 128")		
...
addEventHandler("getStats", getRootElement(), function()
	if DATABASE then
		local data = {}
		recordsQuery = dbQuery(DATABASE, "SELECT * FROM BadlandsTable ORDER BY score ASC LIMIT 128")
