...
function vehicleRandomiser(player, key, keyState)
  ...
  -- this is way more performant of getting a random than indexing into tables
  -- and you know players will spam W
  local newveh = getTickCount() % 211 + 400 
  ...
  setElementPosition(veh, x, y, z + newHeight / 2 - oldHeight / 2)

end

local bindings = {}
function enterVehicle ( theVehicle, seat, jacked ) --trigger on vehicle enter so that late joiners also get bind
    if not bindings[source] then
      bindings[source] = true
      bindKey ( source, "accelerate", "down", vehicleRandomiser )  
      bindKey ( source, "handbrake", "down", vehicleRandomiser )
    end
end
