...
local availablevehicles2 = {
	banshee,bullet,cheetah,comet,elegy,flash,hotknife,hotringracer,hotringracer2,hotringracer3,
	infernus,jester,stratum,sultan,supergt,turismo,uranus,windsor,zr350,
	alpha,blistacompact,bravura,buccaneer,cadrona,club,esperanto,feltzer,fortune,hermes,hustler,
	majestic,manana,picador,previon,stafford,stallion,tampa,virgo,
	admiral,glendaledamaged,elegant,emperor,euros,glendale,greenwood,intruder,merit,nebula,
	oceanic,premier,primo,sentinel,stretch,sunrise,tahoma,vincent,washington,willard,
	bf400,bike,bmx,faggio,fcr900,freeway,mountainbike,nrg500,pcj600,pizzaboy,sanchez,wayfarer,
	baggage,utilityvan,barracks,enforcer,fbirancher,fbitruck,firetruck,firetruckladder,hpv1000,
	patriot,policels,policesf,policelv,policeranger,rhino,swat,securicar,sadler,sadlerdamaged,tug,walton,yosemite,
	blade,broadway,remington,savanna,slamvan,tornado,voodoo,
	buffalo,clover,phoenix,sabre,bandito,bfinjection,bloodringbanger,caddy,camper,
	forklift,journey,kart,mesa,monster,monster2,monster3,quadbike,sandking,vortex,huntley,landstalker,perennial,rancher,rancherlure,regina,romero,solair
} -- don't forget about the trailing comma...

--local carids = {400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 418, 419, 420, 421, 422, 423, 424, 426, 427, 428, 429, 431, 432, 433, 434, 436, 437, 438, 439, 440, 442, 443, 444, 445, 448, 451, 455, 456, 457, 458, 459, 461, 462, 463, 466, 467, 468, 470, 471, 474, 475, 477, 478, 479, 480, 481, 482, 483, 485, 486, 489, 490, 491, 492, 494, 495, 496, 498, 499, 500, 502, 503, 504, 505, 506, 507, 508, 509, 510, 514, 515, 516, 517, 518, 521, 522, 523, 524, 525, 526, 527, 528, 529, 530, 531, 532, 533, 534, 535, 536, 539, 540, 541, 542, 543, 544, 545, 546, 547, 549, 550, 551, 552, 554, 555, 556, 557, 558, 559, 560, 561, 562, 565, 566, 567, 568, 571, 572, 573, 574, 575, 576, 578, 579, 580, 581, 582, 583, 585, 586, 587, 588, 589, 596, 597, 598, 599, 600, 601, 602, 603, 604, 605, 609}
--local carids2 = {429, 541, 415, 480, 562, 565, 434, 494, 502, 503, 411, 559, 561, 560, 506, 451, 558, 555, 477, 602, 496, 401, 518, 527, 589, 419, 533, 526, 474, 545, 517, 410, 600, 436, 580, 439, 549, 491, 445, 604, 507, 585, 587, 466, 492, 546, 551, 516, 467, 426, 547, 405, 409, 550, 566, 540, 421, 529, 581, 509, 481, 462, 521, 463, 510, 522, 461, 448, 468, 586, 485, 552, 433, 427, 490, 528, 407, 544, 523, 470, 598, 596, 597, 599, 432, 601, 428, 543, 583, 478, 554, 536, 575, 534, 567, 535, 576, 412, 402, 542, 603, 475, 568, 424, 504, 457, 483, 508, 571, 500, 444, 556, 557, 471, 495, 539, 579, 400, 404, 489, 505, 479, 442, 458}
local boatids = {472, 473, 493, 595, 484, 430, 453, 452, 446, 454}
local planeids = {460, 476, 511, 512, 513, 519, 520, 593}
...
function randomCar(checkpoint, time)
	if not source then
	  ...
	  local oldHeight = heights[vehmodel]
        if (checkpoint == 5) or (checkpoint >= 7 and checkpoint <= 11) or (checkpoint >= 15 and checkpoint <= 18) then
		...
		if checkpoint == 6 then
			local newcar = availablevehicles2[math.random(#availablevehicles2)]
			setElementModel (veh, newcar)
			local newHeight = heights[newcar]
			local x, y, z = getElementPosition(veh)
			  setElementPosition(veh, x, y, z + newHeight / 2 - oldHeight / 2 + 0.1) -- just have to leave the 0.1 to be safe...
        ...
        if checkpoint == 19 then
            setElementModel (veh, infernus)
			local newHeight = heights[infernus]
			local x, y, z = getElementPosition(veh)
			setElementPosition(veh, x, y, z + newHeight / 2 - oldHeight / 2 + 0.1) -- just have to leave the 0.1 to be safe...
	end
