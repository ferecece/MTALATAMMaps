...
local myTextItem = textCreateTextItem ( "Enemy approaching", 0.5, 0.048, "low", 255, 0, 0, 255, 1, "center")    
...
addEventHandler("onRaceStateChanging", getRootElement(),function ( state, oldstate )	
	...
	racestate = state
	if (state == "Running" and oldstate == "GridCountdown") and (activestate == 1) then
		if startdely == 0 then
			activate()
		else
