		...
		for player in pairs(nametags) do 
			while true do
				...
				if pdistance <= NAMETAG_DISTANCE then
					--Get screenposition
					local sx,sy = getScreenFromWorldPosition ( px, py, pz+1.5, 0.06 )
					...
					local score = getElementData(player,"quizScore")
					if not score then score = 0 end
					dxDrawText ("Score: " .. score, sx, sy - offset, sx, sy - offset, tocolor(r,g,b,textalpha), textscale*NAMETAG_TEXTSIZE, "pricedown", "center", "bottom", false, false, false )		
				end
				break
			end
		end
